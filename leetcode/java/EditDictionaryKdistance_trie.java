 ...
 * 	due to the dictionary, use Trie
 * 
 ...
 * Trie should be solution of preparation of word list. I have a feeling that 
	when the question is about a set of things and a target, the set should be prepared for repeated calls.

 * use bfs with target word. 

 */
public class EditDictionaryKdistance_trie {
    
	// build the Trie Dictionary
    public TrieNodeHMSaveStr constructTire(String[] dict) {
    	
        TrieNodeHMSaveStr root = new TrieNodeHMSaveStr(null, ' ');
        for (String aDict : dict) {
            TrieNodeHMSaveStr ptr = root;
    ...
    // TBD
    public void EditDictionaryKDistance_find_print(String target, TrieNodeHMSaveStr ptr, int k) {
    	
        int m = target.length()+1;			// num of char in target + 1
        int n = TrieNodeHMSaveStr.counter;	// number of char in dictionary + 1
        
        // edit distance matrix
        int[][] matrix = new int[m][n];
        int i,j;
        for(i = 0 ; i < m ; i++)	matrix[i][0] = i;
        for(j = 0 ; j < n ; j++)	matrix[0][j] = TrieNodeHMSaveStr.findNode(j).depth;
        
        
        for(j = 1; j < n ;j++){
            for(i = 1 ; i < m ; i++) {
            	
                TrieNodeHMSaveStr node = TrieNodeHMSaveStr.findNode(j);
    ...
    public static void main(String[] args) {
    	EditDictionaryKdistance_trie outerKD = new EditDictionaryKdistance_trie();
    	
        String[] input = new String[]{"baa","aaa","zz", "abcd", "bad", "adbcd"};
        String target = "a";
        
        TrieNodeHMSaveStr root = outerKD.constructTire(input);
