 ...
 * 		(better algorithm) binary search, O(NlogN) runtime, O(1) space
 * 		(best algorithm) two pointers from two side search: O(n) runtime, O(1) space -- best for already sorted
 *
 *  For two sum, shellSort is optional, not much gain as HashMap is fast enough
 *  For Three/four sum, shellSort should be done first to use faster two pointers algorithm
 */
public class TwoSumThreeSumFourSum_2P {
    ...
    // utility to skip duplicate for move right or left
    ...
    public int[] twoSum_Sorted(int[] num, int target) {
        ...
        while (lo < hi) {
            ...
            if (sum == target) {
                ...
                // question says only one solution possible, done, break now
                break;
            } else if (sum > target) {
    ...
    // Note no need to keep original array and search index since only value is requested
    ...
    public int threeSumClosest(int[] nums, int target) {
        ...
        for (int i = 0; i < nums.length - 2; i = increment(nums, i)) {
            ...
            while (lo < hi) {
                ...
                if (sum == target) {
                    // the question says only one solution, so safe to return here for a match
                    return sum;
                } else if (sum < target) {
